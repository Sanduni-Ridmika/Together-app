...
% load dataset
data=readtable('togetherapp.csv','TextType','string');
...
% Define the mappings from text values to numbers
mapping = containers.Map({'Not at all', 'Several days', 'More than half the days', 'Nearly everyday'}, {0, 1, 2, 3});

% Loop over each column in the dataset
for i = 1:size(DataC, 2)
    % Check if the column contains text values to be mapped
    if iscell(DataC{1,i})
        % Map the text values to numbers
        for j = 2:size(DataC, 1)
            if mapping.isKey(DataC{j,i})
                DataC{j,i} = mapping(DataC{j,i});
            end
        end
    end
end
